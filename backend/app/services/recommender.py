...
MEDIA_SENTIMENT_RUNTIME_DB_PATH = os.path.join(
    PROJECT_ROOT,
    "ai_workspace",
    "Wisata_Workspace",
    "01_Dataset",
...
class MuterBandungRecommender:
    ...
    def __init__(self, db_path='data/wisata_populer.csv'):
        ...
        if db_path == 'DATABASE_WISATA_FINAL_PARIPURNA.csv':
            reviewed_candidates = [
                ...
                MEDIA_SENTIMENT_RUNTIME_DB_PATH,
                os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), RUNTIME_CANDIDATE_DB_PATH),
                os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), MEDIA_SENTIMENT_RUNTIME_DB_PATH),
                REVIEWED_DB_PATH,
        ...
        if "kota_kabupaten" not in self.df.columns:
            # Newer runtime CSVs do not always carry the legacy region column.
            # Mark as unknown so location filters keep candidates instead of crashing.
            self.df["kota_kabupaten"] = "unknown"
        else:
            self.df["kota_kabupaten"] = self.df["kota_kabupaten"].fillna("unknown").astype(str)
    ...
    def _normalize_signal_label(self, value):
        if value is None:
            return ""
    ...
    def _row_personalization_labels(self, row):
        labels = []
        scalar_keys = [
            "category",
            "subcategory",
            "primary_intent_final",
        ...
        list_keys = [
            ...
            "display_labels_parsed",
            "multi_labels_parsed",
        ]
        semicolon_keys = ["final_core_labels", "final_secondary_labels", "final_avoid_labels"]
        ...
        for key in semicolon_keys:
            labels.extend(self._parse_semicolon_labels(row.get(key, "")))

        labels.extend(self._parse_multi_labels(row.get("multi_labels", "")))
        return {label for label in (self._normalize_signal_label(item) for item in labels) if label}

    def _labels_overlap(self, left, right):
        left_norm = self._normalize_signal_label(left)
        right_norm = self._normalize_signal_label(right)
        if not left_norm or not right_norm:
            return False
        return left_norm == right_norm or left_norm in right_norm or right_norm in left_norm

    def _normalize_signal_score(self, value):
        try:
            score = float(value)
    ...
    def _extract_persona_context(self, persona_context):
        if not isinstance(persona_context, dict):
            return {}, {}
        persona = persona_context.get("persona")
        metadata = persona_context.get("metadata")
        if isinstance(persona, dict):
            return persona, metadata if isinstance(metadata, dict) else {}
        return persona_context, {}

    def _compute_persona_signal(self, row, persona_context):
        persona, metadata = self._extract_persona_context(persona_context)
        persona_source = self._clean_scalar_text(persona.get("persona_source", ""))
        model_used = bool(metadata.get("model_used")) or persona_source.startswith("ml_")
        allowed = bool(persona.get("ranking_signal_allowed") or metadata.get("ranking_signal_allowed"))

        signal = {
            "score": 0.0,
            "boost": 0.0,
            "persona_id": self._clean_scalar_text(persona.get("persona_id", "")),
            "persona_source": persona_source,
            "persona_confidence": self._normalize_signal_score(persona.get("persona_confidence")),
            "matched_labels": [],
            "applied": False,
            "model_used": model_used,
        }
        if not (persona and model_used and allowed):
            return signal

        boost_labels = persona.get("home_boost_labels", [])
        if not isinstance(boost_labels, list):
            boost_labels = []
        candidate_labels = self._row_personalization_labels(row)
        matched_labels = []
        for boost_label in boost_labels:
            if any(self._labels_overlap(boost_label, candidate_label) for candidate_label in candidate_labels):
                matched_labels.append(str(boost_label))

        if matched_labels:
            signal["matched_labels"] = sorted(set(matched_labels))
            signal["score"] = signal["persona_confidence"]
            signal["boost"] = round(signal["score"] * 4.0, 4)
            signal["applied"] = True
        return signal

    def _compute_behaviour_signal(self, row, behaviour_context):
        if not isinstance(behaviour_context, dict):
            behaviour_context = {}
        metadata = behaviour_context.get("metadata") or {}
        predictions = behaviour_context.get("next_category_predictions") or []
        if not isinstance(predictions, list):
            predictions = []

        model_source = self._clean_scalar_text(metadata.get("model_source", ""))
        allowed = bool(metadata.get("ranking_signal_allowed"))
        model_available = bool(model_source and model_source != "fallback_empty")
        signal = {
            "score": 0.0,
            "boost": 0.0,
        ...
        if not (model_available and allowed):
            return signal
        ...
        for prediction in predictions:
            ...
            if any(self._labels_overlap(category, candidate_label) for candidate_label in candidate_labels):
                score = self._normalize_signal_score(prediction.get("score"))
                if score > best_score:
                    best_score = score
        ...
        if matched_categories:
            ...
            signal["boost"] = round(best_score * 5.0, 4)
            signal["matched_categories"] = sorted(set(matched_categories))
            signal["applied"] = True
        return signal

    def _build_personalization_metadata(self, persona_context, behaviour_context):
        persona, persona_metadata = self._extract_persona_context(persona_context)
        if not isinstance(behaviour_context, dict):
            behaviour_context = {}
        behaviour_metadata = behaviour_context.get("metadata") or {}
        behaviour_predictions = behaviour_context.get("next_category_predictions") or []
        persona_allowed = bool(persona.get("ranking_signal_allowed") or persona_metadata.get("ranking_signal_allowed"))
        behaviour_allowed = bool(behaviour_metadata.get("ranking_signal_allowed"))
        return {
            "planner_signal_enabled": bool(persona_allowed or behaviour_allowed),
            "persona": {
                "persona_id": self._clean_scalar_text(persona.get("persona_id", "")),
                "persona_source": self._clean_scalar_text(persona.get("persona_source", "")),
                "model_used": bool(persona_metadata.get("model_used")),
                "ranking_signal_allowed": persona_allowed,
            },
            "behaviour": {
                "model_source": self._clean_scalar_text(behaviour_metadata.get("model_source", "")),
                "fallback_used": bool(behaviour_metadata.get("fallback_used")),
                "ranking_signal_allowed": behaviour_allowed,
                "prediction_count": len(behaviour_predictions) if isinstance(behaviour_predictions, list) else 0,
            },
        }
    ...
    def _apply_hard_filters(self, df, categories=None, max_price=None, min_rating=None, free_only=False,
        ...
        if target_location:
            kota_series = filtered_df.get('kota_kabupaten', __import__('pandas').Series('unknown', index=filtered_df.index)).astype(str)
            if isinstance(target_location, list):
                mask_loc = kota_series.isin(target_location)
            else:
                mask_loc = kota_series == target_location
                
            mask_unknown = kota_series.str.lower() == 'unknown'
            mask_missing = kota_series.isna()
    ...
    def _compute_final_score(self, df_filtered, similarity_scores, query=None, weights=None,
                             ...
                             persona_context=None, behaviour_context=None):
        ...
        for idx, row in df_filtered.iterrows():
            ...
            persona_signal = self._compute_persona_signal(row, persona_context)
            behaviour_signal = self._compute_behaviour_signal(row, behaviour_context)
            personalization_boost = persona_signal["boost"] + behaviour_signal["boost"]
            if personalization_boost:
                final_score += personalization_boost
            ...
            results.append({
                ...
                'score_breakdown': {
                    ...
                    'personalization_applied': bool(persona_signal["applied"] or behaviour_signal["applied"]),
                    'persona_score': round(persona_signal["score"], 4),
                    'persona_boost': round(persona_signal["boost"], 4),
                    'persona_applied': bool(persona_signal["applied"]),
                    'persona_model_used': bool(persona_signal["model_used"]),
                    'persona_id': persona_signal["persona_id"],
                    'persona_source': persona_signal["persona_source"],
                    'persona_confidence': round(persona_signal["persona_confidence"], 4),
                    'persona_matched_labels': persona_signal["matched_labels"],
                    'behaviour_score': round(behaviour_signal["score"], 4),
                    'behaviour_boost': round(behaviour_signal["boost"], 4),
                    'behaviour_applied': bool(behaviour_signal["applied"]),
                    'behaviour_model_used': bool(behaviour_signal["model_used"]),
                    'behaviour_model_source': behaviour_signal["model_source"],
                    'behaviour_fallback_used': bool(behaviour_signal["fallback_used"]),
                    'behaviour_matched_categories': behaviour_signal["matched_categories"]
                },
    ...
    def recommend(self, query=None, categories=None, max_price=None,
        ...
        return {
            ...
            "personalization": self._build_personalization_metadata(persona_context, behaviour_context),
            "ai_badge": {
                "enabled": bool(query and query.strip()),
                "label": "Pencarian Cerdas AI",
                "active_intents": detected_intents,
