  ...
  return (
    <main className="mx-auto max-w-[1180px] px-4 py-12 sm:px-8">
      ...
      <div className="mb-12 text-center max-w-2xl mx-auto">
        <h1 className="text-[36px] font-bold tracking-tight text-[#112F43] sm:text-[42px]">
          Perjalananmu Siap!
        </h1>
        <p className="mt-4 text-base text-[#557083]">
          Cepot AI telah menyusun itinerary terbaik berdasarkan preferensi dan budget yang kamu pilih.
        </p>
      </div>
      ...
      <div className="grid gap-10 lg:grid-cols-[340px_minmax(0,1fr)] items-start">
        ...
        <div>
          ...
          <section className="mt-12 mb-12 rounded-[24px] border border-[#CFE5F2] bg-[#F2FAFE] p-8 shadow-sm">
            <div className="flex justify-between items-center mb-8 border-b border-[#CFE5F2] pb-6">
              <h3 className="font-bold text-[22px] text-[#112F43] flex items-center gap-3">
                <ListChecks className="h-6 w-6 text-[#0E75BC]" /> Rangkuman Perjalananmu
              </h3>
              <Link href="/explore" className="bg-white border border-[#0E75BC] text-[#0E75BC] px-5 py-2.5 rounded-full text-xs font-bold shadow-sm hover:bg-slate-50 transition-colors flex items-center gap-2">
                <span className="text-lg leading-none">+</span> Tambah Perjalanan
              </Link>
            </div>
            
            <div className="grid grid-cols-3 gap-6">
              <div>
                <h4 className="text-[11px] font-bold text-[#0E75BC] mb-4 tracking-wider uppercase">DESTINASI TERPILIH</h4>
                {destinations.length > 0 ? (
                  <ul className="space-y-3">
                    {destinations.map((dest, i) => (
                      <li key={i} className="flex items-center gap-2">
                        <span className="h-4 w-4 rounded-full bg-[#112F43] flex items-center justify-center text-[10px] text-white shrink-0">✔</span>
                        <span className="text-sm text-[#557083] font-medium">{dest.title}</span>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className="text-sm text-[#557083]">Belum ada.</p>
                )}
              </div>
              
              <div>
                <h4 className="text-[11px] font-bold text-[#0E75BC] mb-4 tracking-wider uppercase">PENGINAPAN</h4>
                {accommodations.length > 0 ? (
                  <ul className="space-y-3">
                    {accommodations.map((acc, i) => (
                      <li key={i} className="flex items-center gap-2">
                        <span className="h-4 w-4 rounded-full bg-[#112F43] flex items-center justify-center text-[10px] text-white shrink-0">✔</span>
                        <span className="text-sm text-[#557083] font-medium">{acc.name}</span>
                      </li>
                    ))}
                  </ul>
                ) : (
              ...
              <div>
                <h4 className="text-[11px] font-bold text-[#0E75BC] mb-4 tracking-wider uppercase">DURASI</h4>
                <p className="text-sm text-[#557083] font-medium">{durationString}</p>
              </div>
            </div>
          </section>
